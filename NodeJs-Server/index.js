...
io.on('connection', function (socket) {
	console.log(`Đã có tín hiệu vào với id là: ${socket.id}`);
    ...
    //Bình luận blog
    socket.on('blog_create_comment', function (data) {
        console.log(data)
        io.emit('blog_create_comment', data)
    });

    socket.on('blog_edit_comment', function (data) {
        console.log(data)
        io.emit('blog_edit_comment', data)
    });

    socket.on('blog_delete_comment', function (data) {
        console.log(data)
        io.emit('blog_delete_comment', data)
    });

    socket.on('blog_create_comment_detail', function (data) {
        console.log(data)
        io.emit('blog_create_comment_detail', data)
    });
    socket.on('blog_edit_comment_detail', function (data) {
        console.log(data)
        io.emit('blog_edit_comment_detail', data)
    });
    socket.on('blog_delete_comment_detail', function (data) {
        console.log(data)
        io.emit('blog_delete_comment_detail', data)
    });

    socket.on('like_blog', function (data) {
        console.log(data)
        io.emit('like_blog', data)
    }); 
});
